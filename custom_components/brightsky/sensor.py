...
def _get_current_sensor_value(data: dict[str, Any], key: str) -> StateType:
    ...
    if not data or "current" not in data:
        _LOGGER.debug("No 'current' data available for sensor %s", key)
        return None
        
    current_weather = data["current"].get("weather")
    if not current_weather:
        _LOGGER.debug("No 'weather' data in 'current' for sensor %s", key)
        return None
    ...
    if isinstance(current_weather, list) and len(current_weather) > 0:
        ...
        _LOGGER.debug("Using first item from weather list for sensor %s", key)
    elif isinstance(current_weather, dict):
        current_data = current_weather
        _LOGGER.debug("Using weather dict for sensor %s", key)
    else:
        _LOGGER.warning("Unexpected weather data type for sensor %s: %s", 
                        key, type(current_weather))
        return None
        
    if not current_data:
        _LOGGER.debug("No current_data extracted for sensor %s", key)
        return None
    
    # Direkte Schlüssel versuchen
    value = current_data.get(key)
    
    # Wenn der Wert nicht gefunden wurde, nach alternativen Schlüsseln suchen
    if value is None:
        # Fallback für ältere API-Versionen ohne Zeitfenster-Suffixe
        if key.endswith("_10") or key.endswith("_30") or key.endswith("_60"):
            base_key = key.split("_")[0]
            if current_data.get(base_key) is not None:
                _LOGGER.debug("Using fallback key %s instead of %s", base_key, key)
                return current_data.get(base_key)
        
        # Fallback für aktuelle API-Version mit Zeitfenster-Suffixen
        if not (key.endswith("_10") or key.endswith("_30") or key.endswith("_60")):
            for suffix in ["_10", "_30", "_60"]:
                alt_key = f"{key}{suffix}"
                if current_data.get(alt_key) is not None:
                    _LOGGER.debug("Using alternative key %s instead of %s", alt_key, key)
                    return current_data.get(alt_key)
        
        _LOGGER.debug("Key %s not found in weather data. Available keys: %s", 
                    key, list(current_data.keys()))
